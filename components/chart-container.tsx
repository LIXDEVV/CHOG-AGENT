...
export default function ChartContainer({ pairAddress, tokenName }: ChartContainerProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-purple-900/40 rounded-lg border border-purple-800/30">
        <div className="animate-spin rounded-full h-12 w-12 border-2 border-purple-500 border-t-transparent mb-4" />
        <p className="text-purple-300">Loading chart data...</p>
      </div>
    )
  }

  return (
    <div className="w-full h-full flex flex-col bg-purple-900/20 rounded-lg border border-purple-800/30 p-6 overflow-hidden">
      {/* Market Cap Stats */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-purple-900/40 rounded p-3">
          <p className="text-xs text-purple-400 mb-1">Current MC</p>
        ...
        <div className="bg-purple-900/40 rounded p-3">
          <p className="text-xs text-purple-400 mb-1">24h High</p>
        ...
        <div className="bg-purple-900/40 rounded p-3">
          <p className="text-xs text-purple-400 mb-1">24h Low</p>
        ...
        <div className="bg-purple-900/40 rounded p-3">
          <p className="text-xs text-purple-400 mb-1">24h Change</p>
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#6b3f99" />
            <XAxis dataKey="time" stroke="#a78bba" tick={{ fontSize: 12 }} />
            <YAxis stroke="#a78bba" tick={{ fontSize: 12 }} />
            <Tooltip
              contentStyle={{
